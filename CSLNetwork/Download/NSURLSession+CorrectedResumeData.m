...
- (NSData *)getCorrectRequestDataWithData:(NSData *)data
{
    ...
    while ([objectss[1] objectForKey:[NSString stringWithFormat:@"$%ld", (long)i]]) {
        i++;
    }
    
    NSInteger j = 0;
    while ([archive[@"$objects"][1] objectForKey:[NSString stringWithFormat:@"__nsurlrequest_proto_prop_obj_%ld", (long)j]]) {
        ...
        if (obj) {
            [dic setValue:obj forKey:[NSString stringWithFormat:@"$%ld", (long)i + (long)j]];
